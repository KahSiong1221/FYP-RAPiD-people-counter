...
class RAPiD(nn.Module):
    def __init__(self, backbone='dark53', **kwargs):
        ...
        if backbone == 'dark53':
            ...
            self.backbone = backbones.resnet34()
        elif backbone == 'res50':
            self.backbone = backbones.resnet50()
        elif backbone == 'res101':
            self.backbone = backbones.resnet101()
        else:
        ...
        self.branch_L = backbones.YOLOBranch(chL, 18)
        self.branch_M = backbones.YOLOBranch(chM, 18, prev_ch=(chL//2,chM//2))
        self.branch_S = backbones.YOLOBranch(chS, 18, prev_ch=(chM//2,chS//2))
...
class PredLayer(nn.Module):
    ...
    def __init__(self, all_anchors, anchor_indices, **kwargs):
        ...
        if loss_angle == 'period_L1':
            self.loss4angle = losses.period_L1(reduction='sum')
        elif loss_angle == 'period_L2':
            self.loss4angle = losses.period_L2(reduction='sum')
